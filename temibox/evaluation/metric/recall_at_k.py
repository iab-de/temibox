...
class RecallAtK(Metric):
    r"""
    Calculates recall-at-k, i.e. quotient of predicted relevant labels to number of relevant labels
    ...
    @property
    def name(self) -> str:
        return f"recall@k (k={self._k}, min-score={self._min_score:.2f})"
    ...
    def __call__(self,
        ...
        for i, doc in enumerate(documents):
            ...
            matches[i] = len(plabels & tlabels)/max(1, min(self._k, len(tlabels)))
        ...
        if return_dataframe:
            df = DataFrame({"label_count": result["mean_per_true_count"].keys(),
                            "recall_at_k": result["mean_per_true_count"].values(),
                            "overall": result["mean"]}) \
